...
import { demoStats, demoActivityLogs, demoClients } from "@/lib/demo-data";
import { 
  Users, Building2, MessageSquare, Clock, TrendingUp, CheckCircle2, 
  AlertTriangle, Bell, Download, Plus, RefreshCw 
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const statCards = [
    { label: "Total Clients", value: stats.totalUsers, icon: Users, color: "text-blue-500", bgColor: "bg-blue-500/10" },
    ...
    <AdminLayout title="Dashboard Admin" description="Vue d'ensemble de la plateforme">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="bg-slate-900 border-slate-800 lg:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-lg text-white">Clients Actifs</CardTitle>
            <Link href="/admin/clients">
              <Button variant="ghost" size="sm" className="text-amber-500 hover:text-amber-400">
                Gérer →
              </Button>
          ...
          <CardContent>
            <div className="grid gap-4 md:grid-cols-3">
              {demoClients.map((client) => (
                <div key={client.id} className="p-4 bg-slate-800/50 rounded-lg border border-slate-700">
                  ...
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between text-slate-400">
                      ...
                      <span className="text-white">{client.establishmentsCount}</span>
                    </div>
                    <div className="flex justify-between text-slate-400">
                      <span>Plan</span>
                      <span className="text-white">{client.plan}</span>
                    </div>
                  </div>
                </div>
              ))}
